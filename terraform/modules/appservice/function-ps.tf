resource "azurerm_linux_function_app" "datahub_function_ps" {
  name                        = local.func_app_name_ps
  ...
  app_settings = {
    ...
    "datahub_mssql_project_conn_str" = "Server=tcp:${var.sql_server_dns},1433;Database=${var.sql_database_project};Authentication=Active Directory Managed Identity;Encrypt=True;Trusted_Connection=False;"
  }
...
resource "azurerm_monitor_diagnostic_setting" "log_function_ps" {
  name                       = "spdatahub-log-function-ps"
  target_resource_id         = azurerm_linux_function_app.datahub_function_ps.id
...
resource "azurerm_role_assignment" "kv_role_function_function_ps" {
  for_each = local.kv_user_roles

  scope                = var.key_vault_id
  principal_id         = azurerm_linux_function_app.datahub_function_ps.identity.0.principal_id
